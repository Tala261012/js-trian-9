...
function findElementGreaterThan(arr, num) {
  ...
  let res = arr.find((elem) => elem > num);

  if (res) return res;
  else "Числа яке задовільняе умову не знайдено";
}
...
function findLastEvenNumber(arr) {
  ...
  let res = arr.findLast((elem) => elem % 2 === 0);
  if (res) return res;
...
function getPositiveArrayValues(arr) {
  ...
  let resArr = Array.from(iterator);
  return resArr.filter((item) => item > 0);
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let res = arr.slice(-n);
  let sum = res.reduceRight((summa, elem) => summa + elem);
  res.forEach((elem) => {
    console.log(elem);
  });
  return sum;
...
function expandArrayByN(arr, n) {
  ...
  let expandedArray = arr.flatMap((el) => {
    const tempArr = [];
    for (let j = 1; j <= n; j++) {
      tempArr[tempArr.length] = el;
    }
    return tempArr;
  });
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((num, elem) => {
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = arr.map((word) => {
    ...
    word = word.slice(1);
    word = word.padStart(word.length + 1, char);
    return word;
  });
